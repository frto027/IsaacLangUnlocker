...
int WinMain(
	...
	ofn.lpstrTitle = T(L"[仅忏悔龙]请选择以撒主程序以释放补丁", L"[RGON]Please select isaac main program to extract patch.", L"[RGON]아이작 메인 프로그램을 선택해 패치를 적용하세요");
	...
	if (!extract_userenv(file)) {
		...
		), T(L"[RGON]中文补丁错误", L"[RGON]Patch error", L"[RGON]패치 오류"), MB_ICONERROR);
		return 0;
	}
